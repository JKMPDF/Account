...
const MonthlySalesChart: React.FC<MonthlySalesChartProps> = ({ data, currency }) => {
  const [tooltip, setTooltip] = useState<{ visible: boolean; content: string; x: number; y: number }>({ visible: false, content: '', x: 0, y: 0 });

  if (!data || data.length === 0) {
    return <div className="text-center p-4 bg-white dark:bg-slate-800 rounded-lg border border-slate-200 dark:border-slate-700 h-full flex items-center justify-center text-slate-500">No sales data for this period.</div>;
  }
  ...
  return (
    <div className="bg-white dark:bg-slate-800 p-4 sm:p-6 rounded-lg border border-slate-200 dark:border-slate-700 h-full flex flex-col min-h-[350px]">
      <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200 mb-4">Monthly Sales</h3>
          ...
          <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full" onMouseOut={handleMouseOut}>
            ...
            {data.map((point, i) => (
              <circle
                ...
                cy={getY(point.value)}
                r="4"
                className="fill-sky-500 dark:fill-sky-400 stroke-white dark:stroke-slate-800"
                strokeWidth="2"
                onMouseOver={e => handleMouseOver(e, point)}
                aria-label={`Sales for ${point.label}: ${point.value}`}
                role="figure"
              />
            ))}
          </svg>
